            ...
            {links.map((link) => {
                ...
                return (
                    <Link
                        key={link.title}
                        href={link.href}
                        className={clsx("flex h-[48px] items-center  gap-2 rounded-md bg-gray-50  text-sm font-medium hover:bg-sky-100 hover:text-blue-600 flex-none justify-start p-2 px-3 pl-5", {
                            'bg-sky-100 text-blue-600': pathName.includes(link.title.toLowerCase())
                        })}
                    >
